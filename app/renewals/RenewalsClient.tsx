            ...
            {sorted.map((c) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom: '1px solid #E8E8E8' }}
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <button onClick={() => openAction(c)} style={{ padding: '5px 12px', background: days <= 60 ? '#DC2626' : days <= 120 ? '#FFF0E5' : '#F2F2F2', color: days <= 60 ? '#fff' : days <= 120 ? '#EA580C' : '#6B6B6B', borderRadius: 7, fontWeight: 600, fontSize: 11, cursor: 'pointer', border: 'none' }}>
                      {days <= 60 ? 'Jetzt handeln' : days <= 120 ? 'Planen' : 'Im Blick'}
                    </button>
                  </td>
                </tr>
              )
