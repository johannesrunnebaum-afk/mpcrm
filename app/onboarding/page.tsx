          ...
          return (
            <div key={phase} style={{ minWidth: 200, flex: '0 0 200px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                ...
                <span style={{ background: isLast ? '#DCFCE7' : '#F2F2F2', color: isLast ? '#16A34A' : '#6B6B6B', borderRadius: 20, padding: '1px 8px', fontSize: 11, fontWeight: 700 }}>{cards.length}</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {cards.map((o) => {
                  ...
                  return (
                    <Link key={o.id} href={`/kunden/${cust.id}`} style={{ background: '#FFFFFF', border: '1px solid #E8E8E8', borderRadius: 10, padding: 14, cursor: 'pointer', display: 'block' }}>
                      ...
                      <div style={{ width: '100%', height: 4, background: '#F2F2F2', borderRadius: 2, overflow: 'hidden' }}>
                        <div style={{ width: `${(done / total) * 100}%`, height: '100%', background: isLast ? '#16A34A' : '#7C3AED', borderRadius: 2 }} />
                      </div>
                      <div style={{ marginTop: 8 }}><PlanBadge plan={cust.plan} /></div>
                    </Link>
                  )
                })}
                {cards.length === 0 && (
                  <div style={{ border: '2px dashed #E8E8E8', borderRadius: 10, padding: 20, textAlign: 'center', fontSize: 12, color: '#ABABAB' }}>Keine Kunden</div>
                )}
              </div>
            </div>
          )
