...
export default function KundenClient({ customers }: { customers: Customer[] }) {
    ...
    <div style={{ padding: 28, overflow: 'auto', flex: 1 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center' }}>
        ...
        {['Alle', 'Aktiv', 'Gefährdet'].map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{ padding: '7px 14px', borderRadius: 8, border: `1px solid ${filter === f ? '#1A1A1A' : '#E8E8E8'}`, background: filter === f ? '#1A1A1A' : '#FFFFFF', color: filter === f ? '#fff' : '#1A1A1A', fontWeight: filter === f ? 600 : 400, fontSize: 13 }}>
            {f}
          </button>
        ))}
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.map((c) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom: '1px solid #E8E8E8', cursor: 'pointer' }}
                  ...
                  <td style={{ padding: '12px 16px' }}><Link href={`/kunden/${c.id}`}><ChevronRightIcon /></Link></td>
                </tr>
              )
            })}
          </tbody>
